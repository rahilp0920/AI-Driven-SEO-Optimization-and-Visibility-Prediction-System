...
> **Per CIS 2450 TA Ricky Gong's email of 2026-03-29, the project was narrowed from
> the originally-scoped 50,000 rows to ~1,300 developer-documentation pages because
> full-scale free-tier SERP scraping is rate-limited. This is sanctioned, not a
> deviation.**

The narrowing is recorded verbatim in [`data/README.md`](data/README.md) and is
reproduced in the dashboard's About tab. We additionally ship two derivative datasets
under `data/processed/` for the imbalance-handling rubric concept (see
...
`fetch` calls **Brave Search API** (free tier, ~2 K queries/month) by preference
...
results (Source 2) on a normalised `host + path.lower()` key. The
...
|----------|--------------|--------|
| `random_oversample` | Duplicate minority-class rows with replacement until classes are exactly balanced. Cheap, deterministic given the seed; the canonical reference for the rubric concept. | `features_balanced.csv` (1,308 rows, 654 / 654 parity) |
| `bootstrap_augment` | Per-class bootstrap with σ = 0.02 × column-std Gaussian jitter on numeric features; identifier columns copied untouched. Class proportions preserved per class. | `features_augmented.csv` (~52K rows when factor=40) |
...
[`presentation/`](presentation/) contains a fully-reproducible deck and
companion script:

| File | Purpose |
|------|---------|
| [`presentation/slides.pptx`](presentation/slides.pptx) | 13-slide 16:9 deck (~852 KB), generated by `build_slides.py`. The Results-slide winner banner reads metrics live, so it's always in sync with `models/metrics/*.json`. |
| [`presentation/script.md`](presentation/script.md) | Full speaker script with per-slide timings (target 9:00, hard window 8–10), speaker split between Rahil and Ayush, pre-record checklist (cameras on, no sped-up audio, no code on slides), PDF-export one-liner. |
| [`presentation/build_charts.py`](presentation/build_charts.py) | Renders 12 chart PNGs at 300 DPI from `features.csv` + saved metrics. |
| [`presentation/build_slides.py`](presentation/build_slides.py) | Composes the deck via `python-pptx` with a shared theme matching the dashboard. |
| [`presentation/charts/`](presentation/charts/) | Pre-rendered chart assets committed for offline grading. |

To regenerate after any retraining:

```bash
python -m presentation.build_charts
python -m presentation.build_slides
soffice --headless --convert-to pdf --outdir presentation presentation/slides.pptx
```
...
### Section 2 — Intermediate Check-In `(+5, ××)`

| Check-in question | Where it's now answered |
|-------------------|-------------------------|
| Get to know your data — issues + handling plan | [`data/README.md`](data/README.md), the pivot paragraph at the top of this README |
...
#### Concept 1 — Feature Importance · `+4`
...
#### Concept 2 — Hyperparameter Tuning · `+4`
...
### Section 4b — Data Pre-processing & Feature Engineering `(+10, ××)`
...
### Section 5b — Model Assessment & Hyperparameter Tuning `(+8, ××)`
...
### Section 7 — Code Quality & Readability `(+10, ×)`
...
### Section 8 — Application of Course Topics `(+10, ×)`
...
### Section 9 — Quality of Dashboard Demo `(+20, ×)`
...
### Section 11 — Quality of Presentation `(+10, ××)`

Double-deduct, so every penalty path is closed:
...
4. **Training-set link graph is closed.** PageRank is computed only over
   ...
   the open web.
5. **Decision threshold is naive.** `0.5` is the default; real SERPs differ
   in expected positive rate per domain, and a calibrated per-host threshold
   would likely lift F1 a few more points.
